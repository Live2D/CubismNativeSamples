...
LAppSpriteShader::LAppSpriteShader(csmString executeAbsolutePath)
{
    _programId = CreateShader(executeAbsolutePath);
}
...
GLuint LAppSpriteShader::CreateShader(csmString executeAbsolutePath)
{
    ...
    // 各シェーダーのコンパイル
    GLuint vertexShaderId = CompileShader(vertShaderFileName, GL_VERTEX_SHADER);
    GLuint fragmentShaderId = CompileShader(fragShaderFileName, GL_FRAGMENT_SHADER);
