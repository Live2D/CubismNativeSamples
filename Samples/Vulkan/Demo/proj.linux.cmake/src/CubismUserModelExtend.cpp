...
void CubismUserModelExtend::Draw(Csm::CubismMatrix44& matrix)
{
    ...
    GetRenderer<Csm::Rendering::CubismRenderer_Vulkan>()->SetMvpMatrix(&matrix);

    // モデルの描画を命令・実行する
    GetRenderer<Csm::Rendering::CubismRenderer_Vulkan>()->DrawModel();
}
...
void CubismUserModelExtend::SetupTextures()
{
    for (csmInt32 modelTextureNumber = 0; modelTextureNumber < _modelJson->GetTextureCount(); modelTextureNumber++)
    {
        ...
        LAppTextureManager::TextureInfo* texture = _textureManager->CreateTextureFromPngFile(
            texturePath.GetRawString(), VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL,
