...
public:
    ...
    */
     ...
     * @param[in]   wind             ->  ウィンドウ
     */
    void Initialize(GLFWwindow* wind);
    ...
    /**
     * @brief    モデルテクスチャのフォーマットを取得する
     *
     * @return  フォーマット
     */
    VkFormat GetTextureFormat() { return _textureFormat; }
...
private:
    ...
    bool _isSwapchainInvalid;                                   // ウィンドウサイズが変更されたかのフラグ
    const bool _enableValidationLayers = true;                  // 検証レイヤーを有効にするか
    VkDebugUtilsMessengerEXT _debugMessenger;                   // デバッグメッセージを出力するオブジェクト
    Csm::csmUint32 _imageIndex;                                 // 現在のイメージインデックス
    VkFormat _depthFormat;                                      // 深度フォーマット
    const VkFormat _surfaceFormat = VK_FORMAT_B8G8R8A8_UNORM;   // サーフェイス用フォーマット
    const VkFormat _textureFormat = VK_FORMAT_R8G8B8A8_UNORM;   // テクスチャ用フォーマット
    bool _framebufferResized;                                   // フレームバッファのサイズが変わったか
};
