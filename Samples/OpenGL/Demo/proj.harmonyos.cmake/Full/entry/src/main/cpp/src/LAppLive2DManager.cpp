...
void LAppLive2DManager::SetUpModel()
{
    ...
    _modelDir.Clear();
    csmVector<csmString> root = NapiBridgeC::GetAssetList(ResourcesPath);
    for (size_t i = 0; i < root.GetSize(); i++)
    {
        csmString target(root[i]);
        target.Append(MODEL3_JSON, sizeof(MODEL3_JSON) - 1);
        csmString modelPath = csmString(ResourcesPath) + root[i];
        csmVector<csmString> sub = NapiBridgeC::GetAssetList(modelPath.GetRawString());
...
void LAppLive2DManager::ChangeScene(csmInt32 index)
{
    _sceneIndex = index;
    if (DebugLogEnable)
    {
