...
{
    Rendering::CubismRenderer_D3D9* renderer = GetRenderer<Rendering::CubismRenderer_D3D9>();
...
void CubismUserModelExtend::SetupTextures()
{
    ...
    for (csmInt32 modelTextureNumber = 0; modelTextureNumber < _modelJson->GetTextureCount(); modelTextureNumber++)
    {
        ...
        CubismTextureManager::TextureInfo* texture = CubismDirectXRenderer::GetInstance()->_textureManager->CreateTextureFromPngFile(texturePath.GetRawString(), isTextureMult,D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DX_FILTER_BOX | D3DX_FILTER_DITHER_DIFFUSION);
        ...
        if (texture)
        {
            ...
            if (CubismDirectXRenderer::GetInstance()->_textureManager->GetTexture(textureManageId, texture))
            {
                GetRenderer<Rendering::CubismRenderer_D3D9>()->BindTexture(modelTextureNumber, texture);
...
void CubismUserModelExtend::ModelOnUpdate()
{
    ...
    // 必要があればここで乗算
    if (MouseActionManager_Common::GetInstance()->GetViewMatrix())
    {
